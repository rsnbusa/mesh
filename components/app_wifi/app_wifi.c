...
#define RETRIES_WIFI 20

ESP_EVENT_DEFINE_BASE(APP_WIFI_EVENT);
static const char *TAG = "app_wifi";
static const int WIFI_CONNECTED_EVENT = BIT0;
static const int WIFI_FAIL_EVENT = BIT1;
...
static esp_err_t read_random_bytes_from_nvs(uint8_t **random_bytes, size_t *len)
{
    ...
    *len = 0;
    ...
    if ((err = nvs_open_from_partition("fctry", CREDENTIALS_NAMESPACE,
                                NVS_READONLY, &handle)) != ESP_OK) {
        ESP_LOGD(TAG, "NVS open for %s %s %s failed with error %d", "fctry", CREDENTIALS_NAMESPACE, RANDOM_NVS_KEY, err);
        return ESP_FAIL;
    }
...
esp_err_t app_wifi_start(app_wifi_pop_type_t pop_type,char * sname, char *popname)
{
    ...
    //was initialized from main appo
  ...
  esp_err_t err=wifi_prov_mgr_is_provisioned(&provisioned);
    ...
    if (!provisioned) {
         ...
         *      - NULL if not used
         */
        char *pop=(char*)malloc(40);
        if(pop_type != POP_TYPE_NONE)
            pop = get_device_pop(pop_type);
...
//        ESP_ERROR_CHECK(wifi_prov_mgr_start_provisioning(security, pop, service_name, service_key));
    ...
    EventBits_t uxBits;
    uxBits=xEventGroupWaitBits(wifi_event_group, WIFI_CONNECTED_EVENT|WIFI_FAIL_EVENT, false, false, portMAX_DELAY);
    if( uxBits & WIFI_FAIL_EVENT)
        return ESP_FAIL;
    else
        return ESP_OK;
}
